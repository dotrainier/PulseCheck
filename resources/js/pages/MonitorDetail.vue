    ...
    <div class="min-h-screen bg-[#070D1A] text-white">
        ...
        <div class="border-b border-white/8 bg-[#070D1A]/90 backdrop-blur-md">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                <div v-if="loading" class="h-16 flex items-center">
                    <div class="w-6 h-6 border-2 border-amber-500/30 border-t-amber-400 rounded-full animate-spin"></div>
                </div>
                ...
                <div v-else-if="monitor" class="flex items-start justify-between">
                    <div class="flex-1">
                        ...
                        <div class="flex items-start gap-4">
                            <div class="shrink-0 mt-1 relative flex items-center justify-center">
                                <div v-if="monitor.status === 'operational'" class="w-3 h-3 rounded-full bg-green-400 animate-ping absolute"></div>
                                <div class="w-3 h-3 rounded-full" :class="{
                                    'bg-green-400': monitor.status === 'operational',
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-3 mb-2 flex-wrap">
                                    ...
                                    <span class="px-2 py-1 text-xs rounded-md capitalize font-medium" :style="{ fontFamily: 'JetBrains Mono, monospace' }"
                                        :class="{
                                            'bg-green-500/10 text-green-400 border border-green-500/20': monitor.status === 'operational',
                    ...
                    <div class="flex items-center gap-2">
                        <button @click="runCheckNow" :disabled="checking"
                            class="px-3 py-2 bg-white/5 border border-white/8 text-sm font-medium rounded-lg hover:bg-white/10 transition-all flex items-center gap-2">
        ...
        <div v-if="monitor" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                <div class="p-4 bg-[#0D1828] border border-white/8 rounded-lg" :style="{ fontFamily: 'JetBrains Mono, monospace' }">
                    <div class="text-xs text-gray-500 mb-1">Uptime (30d)</div>
                    <div class="text-2xl font-bold text-green-400">{{ monitor.uptime }}%</div>
                </div>
                <div class="p-4 bg-[#0D1828] border border-white/8 rounded-lg" :style="{ fontFamily: 'JetBrains Mono, monospace' }">
            ...
            <div v-if="monitor.track_ssl && monitor.ssl_expiry_date" class="p-5 rounded-lg border"
                :class="{ 'bg-orange-500/5 border-orange-500/20': monitor.ssl_expiring, 'bg-green-500/5 border-green-500/20': !monitor.ssl_expiring }">
                <div class="flex items-start gap-4">
                    <div class="shrink-0 w-10 h-10 rounded-lg flex items-center justify-center"
                        :class="{ 'bg-orange-500/20': monitor.ssl_expiring, 'bg-green-500/20': !monitor.ssl_expiring }">
                        <ShieldCheckIcon class="w-5 h-5" :class="{ 'text-orange-400': monitor.ssl_expiring, 'text-green-400': !monitor.ssl_expiring }" />
                    </div>
                    <div class="flex-1">
                        <h3 class="font-semibold mb-1" :class="{ 'text-orange-300': monitor.ssl_expiring, 'text-green-300': !monitor.ssl_expiring }">
                        ...
                        <div class="text-sm text-gray-400 space-y-1" :style="{ fontFamily: 'JetBrains Mono, monospace' }">
                            ...
                            <div>
                                ...
                                <span :class="{ 'text-orange-400': monitor.ssl_expiring, 'text-green-400': !monitor.ssl_expiring }">
            ...
            <!-- Response Time Chart -->
            <div class="bg-[#0D1828] border border-white/8 rounded-lg p-6">
                ...
                <svg viewBox="0 0 1000 200" class="w-full h-48" xmlns="http://www.w3.org/2000/svg">
                    <line v-for="i in 5" :key="i" x1="0" :y1="i * 40" x2="1000" :y2="i * 40" stroke="rgba(255,255,255,0.04)" stroke-width="1" />
                    <path :d="responseChartPath" fill="url(#responseGradient)" opacity="0.3" />
                    <path :d="responseChartLine" fill="none" stroke="#FBBF24" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
                    <circle :cx="dotX" :cy="dotY" r="5" fill="#FBBF24" class="animate-pulse" />
                    <defs>
            ...
            <div class="bg-[#0D1828] border border-white/8 rounded-lg p-6">
                ...
                <div class="flex items-end gap-1 h-32">
                    <div v-for="(day, idx) in uptimeHistory" :key="idx" class="flex-1 group relative">
                        <div class="w-full rounded-t transition-all cursor-pointer hover:opacity-80"
                            :class="{
                                'bg-green-500': day.uptime === 100,
                                'bg-amber-400': day.uptime >= 99 && day.uptime < 100,
                                'bg-yellow-500': day.uptime >= 95 && day.uptime < 99,
                                'bg-red-500': day.uptime < 95,
                            }"
                            :style="{ height: `${day.uptime}%` }"></div>
                        <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-[#070D1A] border border-white/8 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10"
                ...
                <div class="flex items-center justify-between mt-4 text-xs" :style="{ fontFamily: 'JetBrains Mono, monospace' }">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2"><div class="w-3 h-3 rounded bg-green-500"></div><span class="text-gray-400">100%</span></div>
                        <div class="flex items-center gap-2"><div class="w-3 h-3 rounded bg-amber-400"></div><span class="text-gray-400">99–99.9%</span></div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-[#0D1828] border border-white/8 rounded-lg p-6">
                    ...
                    <div v-else class="space-y-2">
                        <div v-for="check in recentChecks" :key="check.id" class="flex items-center justify-between p-3 bg-[#070D1A] rounded-lg">
                            <div class="flex items-center gap-3 flex-1">
                                <div class="w-2 h-2 rounded-full shrink-0"
                                    :class="{ 'bg-green-400': check.success, 'bg-red-400': !check.success }"></div>
                                <div class="flex-1 min-w-0">
                ...
                <div class="bg-[#0D1828] border border-white/8 rounded-lg p-6">
                    ...
                    <div class="space-y-3">
                        <div v-for="incident in monitorIncidents" :key="incident.id"
                            class="p-4 bg-[#070D1A] rounded-lg border-l-2"
            ...
            <div class="bg-[#0D1828] border border-white/8 rounded-lg p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4" :style="{ fontFamily: 'JetBrains Mono, monospace' }">
                    <div class="p-4 bg-[#070D1A] rounded-lg"><div class="text-xs text-gray-500 mb-1">URL</div><div class="text-sm text-white break-all">{{ monitor.url }}</div></div>
                    <div class="p-4 bg-[#070D1A] rounded-lg"><div class="text-xs text-gray-500 mb-1">Check Interval</div><div class="text-sm text-white">{{ monitor.check_interval }}</div></div>
                    <div class="p-4 bg-[#070D1A] rounded-lg"><div class="text-xs text-gray-500 mb-1">Expected Status Code</div><div class="text-sm text-white">{{ monitor.expected_status_code ?? 'Any 2xx' }}</div></div>
                    <div class="p-4 bg-[#070D1A] rounded-lg"><div class="text-xs text-gray-500 mb-1">Timeout</div><div class="text-sm text-white">{{ monitor.timeout }}s</div></div>
                    <div class="p-4 bg-[#070D1A] rounded-lg"><div class="text-xs text-gray-500 mb-1">SSL Tracking</div><div class="text-sm text-white">{{ monitor.track_ssl ? 'Enabled' : 'Disabled' }}</div></div>
                    <div class="p-4 bg-[#070D1A] rounded-lg"><div class="text-xs text-gray-500 mb-1">Created</div><div class="text-sm text-white">{{ formatDate(monitor.created_at) }}</div></div>
                </div>
            </div>
        </div>
        ...
        <div v-else-if="!loading" class="max-w-7xl mx-auto px-4 py-16 text-center">
            ...
            <button @click="goBack" class="mt-4 text-amber-400 hover:text-amber-300 text-sm">← Back to Monitors</button>
        </div>
    </div>
</template>
